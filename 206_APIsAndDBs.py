...
def checkUser(user):
    ...
    if user in CACHE_DICTION:
        ...
        try:
        	data = api.user_timeline(screen_name = user)
...
conn = sqlite3.connect('206_APIsAndDBs.sqlite', timeout = 5)
cur = conn.cursor()
...
#Umich user
user_id = umich_tweets[0]["user"]["id_str"]
screen_name = umich_tweets[0]["user"]["screen_name"]
num_favs = umich_tweets[0]["user"]["favourites_count"]
description = umich_tweets[0]["user"]["description"]
...
for i in range(20):
	try:
		user_id = umich_tweets[i]["entities"]["user_mentions"][0]["id_str"]
		screen_name = umich_tweets[i]["entities"]["user_mentions"][0]["screen_name"]
		user_info = get_user_tweets(screen_name) 
		num_favs = user_info[0]["user"]["favourites_count"]
		description = user_info[0]["user"]["description"]
...
for i in range(20):
	...
	text_tweet = umich_tweets[i]["text"]
	user_posted = umich_tweets[i]["user"]["id_str"]
	time_posted = umich_tweets[i]["created_at"]
	retweets = umich_tweets[i]["retweet_count"]

	cur.execute("INSERT OR IGNORE INTO Tweets (tweet_id, text_tweet, user_posted, time_posted, retweets) VALUES (?, ?,?,?,?)",
...
for i in favorites_data:
	favorites.append(i[0])


# Make a query using an INNER JOIN to get a list of tuples with 2 
# elements in each tuple: the user screenname and the text of the 
# tweet. Save the resulting list of tuples in a variable called joined_data2.
cur.execute('SELECT Users.screen_name, Tweets.text_tweet FROM Users INNER JOIN Tweets ON Users.user_id = Tweets.user_posted')
...
cur.execute('SELECT Users.screen_name, Tweets.text_tweet FROM Users INNER JOIN Tweets ON Users.user_id = Tweets.user_posted ORDER BY Tweets.retweets DESC')
